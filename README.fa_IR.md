...
### ⛔ گزینه‌های Exclusion
  ...
  |سوییچ                |   توضیح     |
  |---------------------| ---------------------------------------- |
  |`-xa, --exclude-asn <LIST>` |    exclude کردن ASN یا Organization خاص |
  |`-xf, --exclude-file <LIST>`  |  exclude کردن IP یا CIDR های داخل File |
  |`-xr, --exclude-range <LIST>`  | exclude کردن IP یا CIDR به صورت inline |
...
### ⚡ گزینه‌های Performance و Tuning
  ...
  |`--speed-workers <N>`     | تعداد workerهای تست سرعت (بازه: 1-50). |
  |`--speed-buffer <N>`      | اندازه buffer تست سرعت (بازه: 1-100). |
  |`--speed-dl <N>`          | حداقل سرعت دانلود (مثلاً 50kb یا 1mb). برای اجرا به `-vc` نیاز دارد. |
  |`--speed-ul <N>`          | حداقل سرعت آپلود (مثلاً 50kb یا 1mb). برای اجرا به `-vc` نیاز دارد. |
...
### 📤 گزینه‌های خروجی
 ...
 | `--random-sni`      |    تصادفی کردن برچسب اول SNI؛ نیازمند گواهی TLS از نوع wildcard. |
...
مقادیر worker و timeout که به‌صورت صریح تعیین شوند، تنظیمات پروفایل را
override می‌کنند. bufferها در صورت تعیین نشدن، بر اساس تعداد workerها
...
### 🛠️ سایر گزینه‌ها
 ...
 |سوییچ                     |                              توضیح   |
  ...
  | `-p, --port <LIST>`        | پورت‌های `443,2053,2083,2087,2096,8443` یا `all`. |

### ♻️ Resume و بازیابی اسکن

| سوییچ | توضیح |
|---|---|
| `--resume` / `--checkpoint` | فعال کردن checkpoint و بازیابی اسکن. |
| `--new` | همراه `--resume` یک سشن جدید می‌سازد و سشن‌های ذخیره‌شده را نادیده می‌گیرد. |
| `--resume-session <ID>` | انتخاب یک سشن خاص؛ به‌صورت خودکار Resume را فعال می‌کند. |
| `--resume-interval <SEC>` | فاصله ذخیره checkpoint؛ بین 10 تا 86400 ثانیه، پیش‌فرض 60. |
| `--resume-dir <PATH>` | مسیر پوشه checkpoint؛ پیش‌فرض `./resume`. |

اگر فقط `cfscanner --resume` را اجرا کنید، تنظیمات آخرین سشن قابل‌بازیابی
خودکار برگردانده می‌شود. برای انتخاب سشن خاص از `--resume-session <ID>` استفاده
کنید. در شروع تعاملی، `C` برای ادامه، `N` برای اسکن جدید و `D` برای حذف سشن
است. با `-y` فقط در صورت تطابق کامل تنظیمات و فایل نتایج ادامه داده می‌شود.
در اسکن محدود ممکن است بخش کوچکی دوباره اسکن شود؛ اسکن بی‌نهایت وضعیت مولد
قطعی خود را بازیابی می‌کند. اطلاعات محرمانه و template خام Xray ذخیره نمی‌شود.

برای شروع یک اسکن نامحدود جدید با قابلیت Resume، بدون بررسی یا حذف چک‌پوینت‌های
...
این ابزار فقط برای مقاصد آموزشی و تحقیقاتی ساخته شده است. اسکن را فقط روی
سیستم‌ها و رنج‌هایی انجام دهید که مجوز آن را دارید و قوانین محلی را رعایت کنید.
